        ...
        <?php
            ...
            if($result){
                ...
                while($row = mysqli_fetch_array($result)){
                    echo"<b class='dost1' onmouseover='symptomy(".$row['id_npspr'].")' onmouseout='Wyczysc()'>".$row['nazwa']." :?</b><br>";
                    $sql = "SELECT * FROM `kategoria`";
                    $result1 = mysqli_query( $_SESSION['con'], $sql ); 
                    if($result1){
                        while($row2 = mysqli_fetch_array($result1)){
                            echo"       -<b class='dost2'> ".$row2['nazwa_kat'].":</b><br>";
                            ...
                            if($result2){
                                while($row3 = mysqli_fetch_array($result2)){
                                    if(in_array($row3['id_dost'],$_SESSION['dost'])){
                                        echo "<label><input class='dost3' type='checkbox' name=".$i." value=".$row3['id_dost']." id='".$row3['id_dost']."' onChange='zapiszDost(".$row3['id_dost'].")' checked>".$row3['text']."</label><br>";
                                        $i++;
                                    }else{
                                        echo "<label><input class='dost3' type='checkbox' name=".$i." value=".$row3['id_dost']." id='".$row3['id_dost']."' onChange='zapiszDost(".$row3['id_dost'].")'>".$row3['text']."</label><br>";
                                        $i++;
                                    }
            ...
            echo' Uwagi : <textarea  rows="4" cols="50"  name="uwagi" id="uwagi">'.$uwagi.'</textarea><br>';
        ?>
